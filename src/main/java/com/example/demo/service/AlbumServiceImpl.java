...
public class AlbumServiceImpl implements AbstractMusicService<Album, AlbumDTO, AlbumCreateDTO> {
  ...
  public AlbumServiceImpl(
      ...
      LoadChildEntities<Album, SimpleTrackDTO> loadTracksFromAlbum,
      AlbumAddTrackService albumAddTrackService,
  ...
  public List<AlbumDTO> loadAll() {
    return albumRepository.findAll().stream()
        .map(this::returnAlbumDTO)
        .collect(Collectors.toList());
  }

  @Override
  public List<AlbumDTO> laodByFilter(String filterValue) {
    return albumRepository.findByEntityTitleContainingIgnoreCase(filterValue).stream()
        .map(this::returnAlbumDTO)
        .collect(Collectors.toList());
  }
  ...
  public List<AlbumDTO> loadByGenre(String genre) {
    return albumRepository.findAll().stream()
        .map(this::returnAlbumDTO)
        .filter(album -> album.getGenre().getGenre().equalsIgnoreCase(genre))
        .collect(Collectors.toList());
  }
  ...
  public AlbumDTO createEntity(AlbumCreateDTO albumCreateDTO) {
    Genre genre =
        genreRepository.findById(albumCreateDTO.getGenreId()).orElseThrow(RuntimeException::new);
    Artist artist =
        artistRepository.findById(albumCreateDTO.getArtistId()).orElseThrow(RuntimeException::new);
    log.info("Recieved = {}", albumCreateDTO.toString());
    Album newAlbum =
        new Album(
            albumCreateDTO.getName(),
            albumCreateDTO.getDescription(),
            albumAddTrackService.getTracksForAlbum(
                Arrays.asList(albumCreateDTO.getTracks()), genre, artist),
            artist,
            genre,
            albumCreateDTO.getImage());
  ...
  private AlbumDTO returnAlbumDTO(Album album) {
    return new AlbumDTO(
        album.getEntityTitle(),
        album.getDescription(),
        album.getId(),
        loadTracksFromAlbum.loadAllChildEntities(album),
        album.getArtist().getId(),
        album.getGenre().getId(),
        new SimpleArtistOnlyDTO(album.getArtist().getEntityTitle()),
        new SimpleGenreDTO(album.getGenre().getEntityTitle()),
        album.getImage());
  }
}
